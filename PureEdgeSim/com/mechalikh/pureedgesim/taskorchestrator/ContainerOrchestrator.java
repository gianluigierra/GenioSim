...
public abstract class ContainerOrchestrator extends SimEntity {
	...
	protected void assignContainerToComputingNode(Container container, String[] architectureLayers) {
		...
		if (nodeIndex != -1) {
			...
			if(Orchestrator.printDebug && !container.getSharedContainer()) System.out.println("ORCHESTRATORE CLOUD: ho associato " + nodeList.get(nodeIndex).getName() + " al container generato dal " + container.getEdgeDevice(0).getName() + ", shared = " + container.getSharedContainer());
			if(Orchestrator.printDebug && container.getSharedContainer()) System.out.println("ORCHESTRATORE CLOUD: ho associato la richiesta dell'operatore " + container.getAssociatedAppName() + " al dispositivo " + nodeList.get(nodeIndex).getName() + ", shared = " + container.getSharedContainer());
